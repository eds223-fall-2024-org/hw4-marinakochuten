...
For this project, I am determining which Exclusive Economic Zones (EEZ) on the West Coast of the US are best suited to developing marine aquaculture for several species of oysters and Dungeness crabs.
...
Next, I combine all sea surface temperature rasters into one raster stack and match the coordinate reference systems of all files. For this analysis, I use EPSG:4396. 
```{r}
#| code-fold: true
...
eez_oyster_area_vect <- left_join(wc_regions,
  ...
  mutate(rank = paste0(order(order(suitable_area_km2, decreasing = TRUE)), 
                       ": ",           # Rank each zone 1-5 based on suitable area
                       rgn)) |>   
  arrange(rank) |>
...
```{r}
#| code-fold: true
#| code-summary: "Show the code for creating the map"
#| fig-width: 10
#| fig-height: 9.5
...
```{r}
#| code-fold: true
#| code-summary: "Show the code for writing the function"
...
suitable_eez_func <- function(species, min_sst, max_sst, min_depth, max_depth){
  ...
  eez_area_vect <- left_join(wc_regions, eez_area, by = 'rgn_id')  |>
    select(c("rgn", "rgn_id", "area_km2", "suitable_area_km2")) |>
    rename("total_area_km2" = "area_km2") |>
    mutate(rank = paste0(order(order(suitable_area_km2, decreasing = TRUE)), ": ", rgn)) |> 
    arrange(rank) |>
    mutate(suitable_area_label = paste("Suitable area: \n", round(suitable_area_km2), "km2"))
