...
if(st_crs(depth) == st_crs(wc_regions)) {
    ...
    warning("CRSs do not match - transforming now")
    depth <- project(depth, crs = st_crs(wc_regions))
}
...
# Trying with unstacked raster to see if it fixes error
if(st_crs(sst_2008) == st_crs(wc_regions)) {
    print("CRSs match")
} else {
    warning("CRSs do not match - transforming now")
    sst_2008 <- project(sst_2008, crs = st_crs(wc_regions))   # error: [write] unknown option(s): crs
}
