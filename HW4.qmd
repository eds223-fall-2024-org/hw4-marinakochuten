...
eez_oyster_area_vect <- left_join(wc_regions, eez_oyster_area, by = 'rgn_id')  |>
  select(c("rgn", "rgn_id", "area_km2", "suitable_area_km2")) |>
  rename("total_area_km2" = "area_km2") |>
  mutate(rank = paste0(order(order(suitable_area_km2, decreasing = TRUE)), ": ", rgn)) |>   # Rank each zone 1-5 based on suitable area
  arrange(rank) |>
  mutate(suitable_area_label = paste("Suitable area: \n", round(suitable_area_km2), "km2"))
```

Create map of EEZs colored by aquaculture suitability
```{r}
#| code-fold: true
#| code-summary: "Show the code"
...
tm_shape(us_states, bbox = wc_regions) +   # basemap 
  ...
  tm_layout(main.title = "Oyster Aquaculture Suitability - Ranked EEZ Regions",
            legend.outside = TRUE,
            bg.color = "grey100",
            fontfamily = "Times",
...
suitable_eez_func <- function(species, min_sst, max_sst, min_depth, max_depth){
  ...
  suitable <- lapp(c(sst_reclass, depth_reclass), fun = function(sst, depth) sst * depth) # multiplication will only place 1 in areas that have 1 for both sst and depth
  ...
  tm_shape(us_states, bbox = wc_regions) +   # basemap 
    ...
    tm_polygons(col = "rank",
                ...
                legend.show = FALSE) +
    tm_text("suitable_area_label", size = 0.8, ymod = -0.7) +   # Suitable area annotation
    tm_layout(main.title = paste(species, "Aquaculture Suitability - Ranked EEZ Regions"),
              legend.outside = TRUE,
              bg.color = "grey100",
              fontfamily = "Times",
              title.position = c('center', 'center')) +
